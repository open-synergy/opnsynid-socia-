...
class SendgridTemplate(models.Model):
    ...
    def update_templates(self):
        ...
        if not api_key:
            raise exceptions.UserError(_("Missing sendgrid_api_key in conf file"))
    ...
    def get_keywords(self):
        ...
        prefix = (
            params.search([("key", "=", "mail_sendgrid.substitution_prefix")]).value
            or "{"
        )
        suffix = (
            params.search([("key", "=", "mail_sendgrid.substitution_suffix")]) or "}"
        )
