...
class MailTrackingEmail(models.Model):
    ...
    def _compute_clicks(self):
        for mail in self:
            obj_mail_tracking_event = self.env["mail.tracking.event"]
            mail.click_count = obj_mail_tracking_event.search_count(
                [("event_type", "=", "click"), ("tracking_email_id", "=", mail.id)]
            )
    ...
    def _sendgrid_event_type_verify(self, event):
        ...
        if sendgrid_event_type not in self._sendgrid_event_type_mapping:
            _logger.error(
                "Sendgrid: event type '%s' not supported", sendgrid_event_type
            )
            return False
    ...
    def _sendgrid_db_verify(self, event):
        ...
        if odoo_db != current_db:
            _logger.error(
                "Sendgrid: Database '%s' is not the current " "database", odoo_db
            )
            return False
        # OK, DB is current
        return True
            ...
            metadata.update(
                {
                    ...
                    "mobile": user_agent.platform in ["android", "iphone", "ipad"],
                }
            )
        ...
        if message_id:
            tracking = self.search(
                [
                    ...
                    ("recipient", "=ilike", event.get("email")),
                ],
                limit=1,
            )
        return tracking
        ...
        if res == "NONE" and is_json:
            for event in request.jsonrequest:
                ...
                if res == "OK":
                    sendgrid_event_type = event.get("event")
                    mapped_event_type = (
                        self._sendgrid_event_type_mapping.get(sendgrid_event_type)
                        or event_type
                    )
                ...
                if res != "NONE":
                    if event_type:
                        _logger.info(
                            "sendgrid: event '%s' process '%s'", event_type, res
                        )
                    else:
