...
class MailMail(models.Model):
    ...
    tracking_email_ids = fields.One2many(
        "mail.tracking.email", "mail_id", string="Registered events", readonly=True
    )
    click_count = fields.Integer(compute="_compute_tracking", store=True, readonly=True)
    opened = fields.Boolean(compute="_compute_tracking", store=True, readonly=True)
    ...
    @api.depends(
        "tracking_email_ids",
        "tracking_email_ids.click_count",
        "tracking_email_ids.state",
    )
    def _compute_tracking(self):
        for email in self:
            click_count = sum(email.tracking_email_ids.mapped("click_count"))
            opened = self.env["mail.tracking.email"].search_count(
                [("state", "=", "opened"), ("mail_id", "=", email.id)]
            )
    ...
    def send_sendgrid(self):
        ...
        api_key = self.env.user.company_id.sendgrid_api_key
        ...
        for email in outgoing:
            ...
            if status == STATUS_OK:
                ...
                email.write({"sent_date": fields.Datetime.now(), "state": "sent"})
    ...
    def _prepare_sendgrid_data(self):
        ...
        for substitution in self.substitution_ids:
            personalization.add_substitution(
                Substitution(substitution.key, substitution.value.encode("utf-8"))
            )
